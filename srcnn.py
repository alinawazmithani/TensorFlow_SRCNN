...
weights = {
    'w1': tf.Variable(tf.random_normal([9, 9, c_dim, 64], stddev=1e-3), name='w1'),
...
#tried to visualize the weights
def model_weights_visualize():
    for i in range(0,64):
        plt.subplot(8,8,i+1)
        plt.imshow(w1_[:,:,0,i], interpolation="nearest", cmap="gray")
    
    for i in range(0,32):
        plt.subplot(4,8,i+1)
        plt.imshow(w2_[:,:,0,i], interpolation="nearest", cmap="gray")

    for i in range(0,32):
        plt.subplot(4,8,i+1)
        plt.imshow(w3_[:,:,i,0], interpolation="nearest", cmap="gray")
...
output_1 = sess.run(conv2, feed_dict={inputs: input_})
output_2 = sess.run(conv1, feed_dict={inputs: input_})
#tried to visualize the feature map
def feature_map_visualize():
    for i in range(0,32):
        plt.subplot(4,8,i+1)
        plt.imshow(output_1[0,:,:,i], interpolation="nearest", cmap="gray")
    for i in range(0,64):
        plt.subplot(8,8,i+1)
        plt.imshow(output_2[0,:,:,i], interpolation="nearest", cmap="gray")
...
groudtruth_image = groudtruth_image.astype(np.float32)
output_=np.squeeze(output_)
print("PSNR SRCNN:")
print(skimage.measure.compare_psnr(groudtruth_image[6:249,6:249], output_))
scipy.misc.imsave("groundtruthimage.png",groudtruth_image)
scipy.misc.imsave("blurredimage.png",blurred_image)
scipy.misc.imsave("srimage.png",output_)    